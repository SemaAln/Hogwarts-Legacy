...
public class Wizard implements MagicSource, Trader, MagicEffectRealization {
	...
	public Wizard(String name, MagicLevel level, int basicHP, int HP, int basicMP, int MP, int money,
		...
		if (level == null) {
			throw new IllegalArgumentException("level cannot be nuull");
		}
		...
		this.knownSpells = new HashSet<Spell>();
		this.knownSpells.addAll(knownSpells);

		this.protectedFrom = new HashSet<AttackingSpell>();
		this.protectedFrom.addAll(protectedFrom);

		this.inventory = new HashSet<Tradeable>();
		this.inventory.addAll(inventory);
		
		if (inventoryTotalWeight() > carryingCapacity) {
			throw new IllegalArgumentException("invetery too heavy");
		}
	...
	public boolean castSpell(Spell s, MagicEffectRealization target) {
		if (s == null || target == null) {
			throw new IllegalArgumentException("spell, target cannotr be null");
		}
		if (isDead()){
			return false;
		}
	...
	public boolean castRandomSpell(MagicEffectRealization target) {
		...
		List<Spell> list = new ArrayList<Spell>(knownSpells);
		Random random = new Random();
		int randomIndex = random.nextInt(list.size());
		Spell s = list.get(randomIndex);
	...
	public boolean useRandomItem(MagicEffectRealization target) {
		...
		List<Tradeable> list = new ArrayList<Tradeable>(inventory);
		Random random = new Random();
		int randomIndex = random.nextInt(list.size());
		Tradeable item = list.get(randomIndex);
	...
	public String toString() {
		...
		return "[" + name + "(" + level + "): " + HP + "/" + basicHP + " " + MP + "/" + basicMP + "; " + money + " " + currency + "; knows " + knownSpells + "; carries " + inventory + "]";
	}
	...
	public boolean provideMana(MagicLevel levelNeeded, int manaAmount) {
		if (levelNeeded == null || manaAmount < 0) {
			throw new IllegalArgumentException("levelNeeded cannot be null, manaamount cannot be negative");
		}
	...
	public boolean earn(int amount) {
		if (amount < 0) {
			throw new IllegalArgumentException("amount canot be negative");
		}
	...
	public boolean addToInventory(Tradeable item) {
		if (item == null) {
			throw new IllegalArgumentException("ite cannot be nulll");
		}
	...
	public boolean loot(Trader looter) {
		...
		Set<Tradeable> itemsc = new HashSet<>(inventory);
		
		for (Tradeable i : itemsc) {
			removeFromInventory(i);
			
			if (looter.addToInventory(i)) {
				transferred = true;
			}
		}
	...
	public void takeDamagePercent(int percentage) {
		if (percentage < 0 || percentage > 100) {
			throw new IllegalArgumentException("percentagee between 0 and 100");
		}
		
		int wert= (int) (basicHP * percentage / 100.0);
		HP = Math.max(0, HP - wert);
	}
	...
	public void weakenMagic(int amount) {
		if (amount < 0) {
			throw new IllegalArgumentException("amount cannt be negative");
		}
	...
	public void weakenMagicPercent(int percentage) {
		if (percentage < 0 || percentage > 100) {
			throw new IllegalArgumentException("percentage between 0 and 100");
		}
		
		int wert = (int) (basicMP * percentage / 100.0);
		MP = Math.max(0, MP - wert);	
	}
	...
	public boolean isProtected(Spell s) {
		if (s == null) {
			throw new IllegalArgumentException("spell cannnt be null");
		}
